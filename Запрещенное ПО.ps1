<#
.SYNOPSIS
    Комплексное решение для анализа запрещенного программного обеспечения в домене Active Directory
.DESCRIPTION
    Скрипт выполняет сбор информации об установленном ПО на компьютерах домена, 
    сравнивает со списком нежелательного ПО, формирует подробные отчеты 
    и осуществляет рассылку пользователям и администратору. 
 .NOTES   
    Требования: PowerShell 5.1+, права администратора домена
#>
...
#region Функция сбора данных по установленному ПО
...
#region Импорт списка запрещенного ПО из CSV
...
#region Поиск запрещенного ПО в предоставленном списке
...
#region Получение списка ПЭВМ из AD
function Get-DomainComputers {
    ...
    try {
        # Используем ADSI для получения компьютеров из домена        
...
#region Получение активных сессий пользователей на ПЭВМ
...
#region Взятие E-mail пользователя из AD В User
...
#region Формирование HTML - отчёта
...
$textBoxAdminEmail.Text = "konstantinov@local.corp"
...
#region Добавляем элементы на форму
